...
void EmitRailMove::Init(SrvManager* srvManager) {
    ...
    // レールの初期化（オブジェクト数を指定）
    rail_.Init(srvManager,100);
    railMoveSpeed_ = 0.0f;
}

void EmitRailMove::Update(const std::vector<Vector3>& controlPos) {
...
// レール上のオブジェクトを描画
void EmitRailMove::RailDraw(const ViewProjection& viewProjection) {
    rail_.Draw(viewProjection);
}

void EmitRailMove::Debug() {
#ifdef _DEBUG
