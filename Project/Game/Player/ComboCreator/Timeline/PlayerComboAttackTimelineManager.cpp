...
void PlayerComboAttackTimelineManager::SetupDefaultTracks() {
    ...
    // 終了待機時間トラック
    {
        ...
        timeline_->AddKeyFrame(trackIdx, moveEndFrame, 1.0f, static_cast<float>(waitDuration), "終了待機時間");
...
void PlayerComboAttackTimelineManager::UpdateFinishWaitKeyFramePosition() {
    if (!timeline_)
        return;
